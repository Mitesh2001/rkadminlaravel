    ...
    <ul class="nav navbar-nav navbar-right">
        <li class="">
            <a href="#" class="dropdown-toggle topbar-user__head" data-toggle="dropdown">
                <span>@lang('Hi'),</span>
                <span class="topbar-user__name">{{ auth()->guard('admin')->user()->firstname }} {{ auth()->guard('admin')->user()->surname }}</span>
                <img src="{{ auth()->guard('admin')->user()->avatar }}" class="topbar-user__image">
            </a>
            <ul class="dropdown-menu topbar-user__dropdown-menu">
                <div class="topbar-user__content-header">
                    <div class="topbar-user__card-wrapper">
                        <div class="topbar-user__card-image-wrapper">
                            <img src="{{ auth()->guard('admin')->user()->avatar }}" class="topbar-user__card-image">
                        </div>
                        <div class="topbar-user__card-details">
                            <div class="topbar-user__card__name">
                                {{ auth()->guard('admin')->user()->firstname }} {{ auth()->guard('admin')->user()->surname }}
                            </div>
                ...
                <ul class="topbar-user__list-wrapper">
                    <li class="topbar-user__list">
                        <a href="{{url('/users', auth()->guard('admin')->user()->id)}}" class="topbar-user__list-link">
                    ...
                    <li class="topbar-user__list">
                        <a href="{{url('/users/' .auth()->guard('admin')->user()->id . '/edit')}}" class="topbar-user__list-link">
                            <span class="user__list-icon">
                                 <i class="fa fa-cog"></i>
                            </span>
                            <span class="user__list-text">
                    ...
                    <li class="topbar-user__list">
                        <a href="{{url('/rkadmin/logout')}}" class="btn btn-outline-metal btn-hover-brand btn-upper btn-font-dark btn-sm btn-bold" style="margin-top: 15px;
