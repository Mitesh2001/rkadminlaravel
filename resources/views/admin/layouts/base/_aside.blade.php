...
<div class="aside aside-left {{ Metronic::printClasses('aside', false) }} d-flex flex-column flex-row-auto" id="kt_aside">
    ...
    <div class="brand flex-column-auto {{ Metronic::printClasses('brand', false) }}" id="kt_brand">
        <div class="brand-logo">
            <a href="{{ url('/rkadmin') }}">
                <img alt="{{ \Helper::getSetting('app_name') ?? "RKADMIN" }}" style="max-height:80px" src="{{ asset('media/logos/'.$kt_logo_image) }}"/>
            </a>
        </div>
    ...
    <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
        ...
        @if (config('layout.aside.self.display') === false)
            <div class="header-logo">
                <a href="{{ url('/') }}">
                    <img alt="{{ \Helper::getSetting('app_name') ?? "RKADMIN" }}" src="{{ asset('media/logos/'.$kt_logo_image) }}"/>
                </a>
            </div>
        @endif
